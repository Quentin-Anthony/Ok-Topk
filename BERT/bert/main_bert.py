...
def load_checkpoint(checkpoint_dir, stage, epoch):
    assert os.path.isdir(checkpoint_dir)
    checkpoint_file_path = os.path.join(checkpoint_dir,
                                        "checkpoint.%d.pth.tar.epoch.%d" % (stage, epoch))
    print("Loading checkpoint from %s" % checkpoint_file_path)
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        indx = item
        while True:
            t1, t2, is_next_label = self.random_sent(indx)
...
def convert_example_to_features(example, max_seq_length, tokenizer):
    ...
    assert len(input_ids) == max_seq_length, "not equal to max len"
    assert len(input_mask) == max_seq_length, "not equal to max len"
    assert len(segment_ids) == max_seq_length, "not equal to max len"
    assert len(lm_label_ids) == max_seq_length, "not equal to max len"
...
def main():
    ...
    assert (args.pipedream and args.dataparallel) is False
    ...
    for stageID in range(nstages):
        flush_group_ranks = stage2rank_map[stageID]
        flush_group_size = float(len(flush_group_ranks))
        print("stageID: ", stageID, "flush_group_ranks: ", flush_group_ranks, "flush_group_size: ", flush_group_size)
    ...
    #num_train_optimization_steps = 450000
    num_train_optimization_steps = 286720
    if args.do_train:
    ...
    if args.pipedream:
        optimizer = BertAdamWithStashing(r.modules(),
                                         ...
                                         verbose_freq=args.verbose_frequency)
    elif args.dataparallel or args.no_input_pipelining:
    ...
    if args.fp16:
        ...
        if args.dataparallel or args.no_input_pipelining:
    ...
    if not args.dataparallel and not args.no_input_pipelining:
    ...
    if args.do_train:
        ...
        print("local batch size: ", args.train_batch_size, "num_replicas: ", num_replicas, "sampler ranks: ", configuration_maps['stage_to_rank_map'][0])
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            n = len(train_loader)
            print("Number of iterations in one epoch: ", n, "update_interval", update_interval)
            if args.num_minibatches is not None:
            ...
            if args.dataparallel:
                #checkpoint = load_checkpoint(args.checkpoint_dir, r.stage, 0)
